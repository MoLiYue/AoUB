...
void printview(int height, int wide, int temp1) {
  ...
  for (j = 0; j < height; j++) {
    for (i = temp1; i < wide; i++) {
      ...
      if (map[j][i] == EDGE)
        printf("-");
      else if (map[j][i] == head_value || map[j][i] == body_value)
        printf("*");
      else if (map[j][i] == cone1)
        printf("^");
      else if (map[j][i] == cone2)
        printf("v");
      else if (map[j][i] == block1)
        printf("=");
      else if (map[j][i] == block2)
        printf("|");
      else if (map[j][i] == item_player_health)
        printf("*");
      else if (((map[j][i] == boss0_body_value) ||
                (map[j][i] == boss0_head_value)) &&
               boss0_exist != 0)
        printf("@");
      else if (map[j][i] == item_gun)
        printf("?");
      else
        printf(" ");
    }
