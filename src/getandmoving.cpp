...
// int player[screen_height][screen_length];
// //新增一个小人数组,未考虑屏幕移动故其大小与当前屏幕数组screen相同

char prev_direction =
    'i'; //-------------新:新增全局变量pre_direction,即前一个操作的方向
char direction = 'i';
char ch1 = '0';
char ch2 = '0';
...
void moving_part(int map[screen_height][map_length]) {
  ...
  case 'a': // 方向键左且不越界
    ...
    if (prev_direction == 'w' || prev_direction == 'x' ||
        prev_direction == 'k') // 新:若前一个操作为跳跃(方向键上)或跳跃后无新按键输入,使pre_direction
                               // 的值变为k
    { // 对应跳跃后仍左右移动的情况
      prev_direction = 'k';
    }
